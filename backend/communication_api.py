...
def sendgrid_webhook():
    ...
    try:
        ...
        for event in webhook_data:
            ...
            from communication_models import EngagementType
            engagement_map = {
                'open': EngagementType.OPEN.value,      # 'open'
                'click': EngagementType.CLICK.value,    # 'click'
                'bounce': EngagementType.BOUNCE.value,  # 'bounce'
                'unsubscribe': EngagementType.UNSUBSCRIBE.value  # 'unsubscribe'
            }
        ...
        if result.get('success'):
            ...
            if result.get('type') == 'status_update':
                ...
                if recipient:
                    ...
                    from communication_models import EngagementType
                    status_map = {
                        'delivered': EngagementType.SMS_DELIVERED.value,  # 'sms_delivered'
                        'read': EngagementType.SMS_READ.value,            # 'sms_read'
                        'received': EngagementType.SMS_READ.value         # 'sms_read'
                    }
