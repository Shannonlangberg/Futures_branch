...
def has_serving_permission(permission_type: str, team_id: str = None) -> bool:
    ...
    if current_user.has_permission('serving', 'admin'):
        return True
...
def log_serving():
    ...
    try:
        ...
        # Update engagement profile if person exists
        ...
        if person and person.engagement_profile:
            person.engagement_profile.add_serving_record(
                role=record.role.name if record.role else 'Unknown',
                location=record.team.name if record.team else 'Unknown',
                campus=record.team.campus if record.team else 'Unknown',
    ...
    try:
        ...
        mock_templates = [
            ...
            },
            {
                'id': '2',
                'name': 'Youth Service',
                'description': 'Youth-focused service template',
                'campus': 'Paradise',
                'service_type': 'YOUTH',
                'estimated_duration_minutes': 75,
                'structure': {
                    'items': [
                        { 'type': 'worship', 'duration': 15, 'description': 'Youth Worship' },
                        { 'type': 'message', 'duration': 25, 'description': 'Youth Message' },
                        { 'type': 'activities', 'duration': 20, 'description': 'Group Activities' },
                        { 'type': 'worship', 'duration': 10, 'description': 'Closing Worship' },
                        { 'type': 'dismissal', 'duration': 5, 'description': 'Dismissal' }
                    ]
                }
            }
...
@serving_bp.route('/templates', methods=['POST'])
@login_required
def create_service_template():
    """Create a new service template"""
    try:
        if not has_serving_permission('write'):
            return jsonify({'error': 'Insufficient permissions'}), 403
        
        data = request.get_json()
        
        # Validate required fields
        required_fields = ['name', 'campus', 'service_type', 'estimated_duration_minutes']
        for field in required_fields:
            if not data.get(field):
                return jsonify({'error': f'Missing required field: {field}'}), 400
        
        # For now, return success - replace with database integration
        new_template = {
            'id': str(uuid.uuid4()),
            'name': data['name'],
            'description': data.get('description', ''),
            ...
            'structure': data.get('structure', { 'items': [] })
        }
...
@serving_bp.route('/templates/<template_id>/apply', methods=['POST'])
@login_required
def apply_service_template(template_id):
    """Apply a service template to create a service plan"""
    try:
        if not has_serving_permission('write'):
            return jsonify({'error': 'Insufficient permissions'}), 403
        ...
        # Validate required fields
        required_fields = ['title', 'campus', 'service_date', 'service_time']
        for field in required_fields:
            if not data.get(field):
                return jsonify({'error': f'Missing required field: {field}'}), 400
        ...
        logger.error(f"Error applying service template: {e}")
        return jsonify({'error': 'Failed to apply template'}), 500

@serving_bp.route('/plans', methods=['GET'])
@login_required
def get_service_plans():
    ...
    try:
        ...
        mock_plans = [
            {
                'id': '1',
                'title': 'Sunday Service - August 25',
                'campus': 'Paradise',
            ...
            {
                'id': '2',
                'title': 'Youth Service - August 26',
                'campus': 'Paradise',
                'service_date': '2025-08-26',
                'service_time': '18:00',
                'template_id': '2',
                'estimated_duration_minutes': 75,
                'notes': 'Youth group service',
                'status': 'draft'
            }
        ]
...
@serving_bp.route('/plans', methods=['POST'])
@login_required
